...
public class JwtUtil {
    ...
    public static String createJWT(long ttlMillis, LoginDTO loginDto) {
        ...
        claims.put("id", loginDto.getUserid());
        ...
        // 生成签发人
        String subject = loginDto.getLogin();
    ...
    public static Boolean isVerify(String token, LoginDTO loginDto) {
        ...
        // 先验证token跟session的用户id是否相同
        if (claims.get("id").equals(loginDto.getUserid())) {
            // 再验证密码是否相同
            if (claims.get("password").equals(loginDto.getPassword())) {
                return true;
            }
        }
        return false;
